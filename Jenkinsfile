			...
			steps {
				script {
					if (params.eksctl_action == 'create') {
						sh 'eksctl create cluster --name devsecops-buggy-app --region us-east-1 --zones us-east-1a,us-east-1b --nodegroup-name linux-buggy-app --nodes 2 --instance-types t2.nano --spot --tags "app=buggy-app" --version 1.25'
					} else {
